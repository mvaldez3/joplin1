...
def randpro_select(prototype_file, random_seed = None):
	...
	return np.array(proto_file.sample(n=1, random_state=random_seed))
...
def stim_build(stim_file, random_prototype_selection):
	...
	if np.array_equal(random_prototype_selection, all1proto) is True:
		...
		csv_file = id_df.to_csv('/Volumes/shares/Cabi/exp/joplin/joplin1/data/_stimfile.csv', header=True, index=False)
		...
		for i in num_rows:
			...
			new_code = compare * 1
			print(new_code)
			# compare the new_code to the prototype to get a sum of the number of features that match so we can categorize
			compare_cat = randpro_arr == new_code

			# convert the boolean phrases to integers so we can sum
			cat = compare_cat * 1

			# sum across the new code to get the number of features that match the prototype. Append this to the sum_feat array
			sums = np.sum(cat)
...
if __name__ == '__main__':
	randpro = randpro_select('/Volumes/shares/Cabi/exp/joplin/joplin1/stim_files/Prototype.csv', None)
	print(randpro)
	stim_build('/Volumes/shares/Cabi/exp/joplin/joplin1/stim_files/train_file.csv', randpro)
