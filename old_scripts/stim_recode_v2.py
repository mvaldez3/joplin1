...
proto_file = pd.read_csv('/Volumes/shares/Cabi/exp/joplin/joplin1/csv_files/Prototype.csv', header = None)
...
train_file = pd.read_csv('/Volumes/shares/Cabi/exp/joplin/joplin1/csv_files/dev_train.csv', header = None)
...
if np.array_equal(randpro, all1proto) == True:
	...
	id_df['category'] = ''
	id_df.columns = ['stim_id', 'sums_feat', 'category']

	#specify parameters for categorization 

	id_df.loc[id_df.sums_feat > 5, 'category'] = 'a'
	id_df.loc[id_df.sums_feat < 5, 'category'] = 'b'
	
	print(id_df)

	#convert to csv
	csv_file = id_df.to_csv(path_or_buf = '/Volumes/shares/Cabi/exp/joplin/joplin1/csv_files/practice.csv', header = True, index = False)

elif np.array_equal(randpro, all0proto) == True:
	sums_feat = []
	recoded_id = []
